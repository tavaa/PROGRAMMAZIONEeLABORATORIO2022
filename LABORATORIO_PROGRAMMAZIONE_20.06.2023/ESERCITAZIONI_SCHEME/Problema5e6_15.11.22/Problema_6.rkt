...
;;
;;   (shift-down <figura> <passi>) ==> sposta la figura in basso di n passi
;;
;;   (shift-right <figura> <passi>) ==> sposta la figura a destra di n passi
;;
;;   (quarter-turn-right <figura>) ==>  ruota di 90 verso dx
;;
;;   (quarter-turn-left <figura>) ==> ruota di 90 verso sx
;;
;;   (half-turn <figura>) ==> ruota di 180
;;
;;   (glue-tiles <figura> <figura>) ==> incolla fig1 a fig2
;;
...
;idea di base:
;Colleggo due figure, le due L ai lati con i due L interni con un unico glue-tiles che collega tutto

(define L-tessellation
  (lambda (a) ;a = potenza di due/intero 
          ;controllo che la lunghezza di a sia maggiore di 1
          (if(= a 1)
             ;se vero
             L-tile ;; ==> caso base, ritorno semplicemente il tassello
             ; altrimenti ( a != 1 )
             (glue-tiles
              ;L interni
                        (glue-tiles
                          (L-tessellation (quotient a 2))
                          (shift-right (shift-down (L-tessellation (quotient a 2)) (quotient a 2))(quotient a 2))
                         )
               ;L esterni
                         (glue-tiles
                         (shift-right (quarter-turn-right (L-tessellation (quotient a 2))) a)
                         (shift-down (quarter-turn-left (L-tessellation (quotient a 2))) a)
                         )  
              )
            )
    )
