...
public class SList<T> {                    // Scheme-Like Lists of T
  ...
  public boolean equals( SList<T> tl ) {   // contronto di liste
                                           // Scheme: equal?
    if ( this.isNull() || tl.isNull() ) {
      return ( this.isNull() && tl.isNull() );
    } else if ( this.car().equals(tl.car()) ) { // equals di tipo T degli elementi della lista
      return ( this.cdr() ).equals( tl.cdr() ); // equals come ricorsione
    } else {
      return false;
    }
  }
  ...
  // ----- Verifica se l'elemento è all'interno della lista
